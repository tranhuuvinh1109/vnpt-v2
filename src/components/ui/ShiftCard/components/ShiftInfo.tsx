        ...
        <div className={`flex flex-col gap-1 ${isEditMode ? "flex-1" : ""}`}>
          ...
          {isEditMode ? (
            ...
            <div className="flex flex-col gap-1">
              <p className={`text-justify text-xs md:text-sm ${isShowMore ? "" : "line-clamp-3"}`}>
                {data.note}
              </p>
              {data.note?.length > 100 && (
                <button
                  onClick={toggleShowMore}
                  className="text-element-primary flex items-center gap-1 text-xs font-medium md:text-sm"
                >
