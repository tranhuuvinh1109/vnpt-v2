...
import { ShiftDetailType } from "@/types";
import { useGetAllMember, useGetAllStation, useUpdateShiftForDayFormData } from "@/api";
...
export const CreateShiftStep = ({ data, refetch, onClose }: CreateShiftStepProps) => {
  ...
  const memberOptions = allMember
    ? allMember?.map((item) => ({ value: item._id ?? "", label: item.email }))
    : [];
  const stationOptions = allStation
    ? allStation?.map((item) => ({ value: item._id ?? "", label: item.address }))
    : [];
  ...
  const intiData: DataShiftForDayRequestType = {
    ...
    assign: data?.assign_user
      ? data?.assign_user.map((item) => ({ value: item._id ?? "", label: item.email }))
      : [],
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (infoData.infor_during?.note)
        formData.append("infor_during_note", infoData.infor_during.note);
      ...
      if (infoData.infor_exist?.note)
        formData.append("infor_exist_note", infoData.infor_exist.note);
          ...
          {activeStep === steps.length ? (
            <div className="flex flex-col gap-4 px-2 py-6 md:py-2">
              <p className="text-center text-lg font-medium">
                Bạn có muốn lưu thông tin ca không?{" "}
              </p>
